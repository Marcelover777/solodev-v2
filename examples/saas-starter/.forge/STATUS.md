...
> Painel de estado. Vive em `.forge/STATUS.md`, escrito pelo `/dev-status` — **derivado de arquivos reais** (`ROADMAP.md`, `git status`, resultado dos testes), nunca de número inventado. Rode `/dev-status` a qualquer hora pra atualizar.
...
> Resumo narrativo do que já rolou — lido do `.forge/PROGRESS.md`. Sem números de token (não há DB; é tudo arquivo).
