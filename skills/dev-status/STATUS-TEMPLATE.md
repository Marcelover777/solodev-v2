...
Salvar em `.forge/STATUS.md` (na raiz do projeto do usuário). Sobrescrito a cada `/dev-status`. Reflete só o que arquivo real sustenta — célula sem fonte é `❓ sem dado`, nunca um valor inventado.
...
- **Sem token-economics.** Este painel não tem custo/token — Forger é file-based, não há DB pra contar.
